...
import {
  ...
  Settings, LogOut, Loader2, RefreshCw, CheckCircle,
  XCircle, AlertCircle, Calendar, ChevronRight, X,
  ExternalLink, FileText, Tag, Banknote, CheckCheck,
} from 'lucide-react';
...
// ── Offer detail Accordion content ───────────────────────────────────────────
    ...
    <div className="p-5 sm:p-6 bg-slate-50/50 border-t border-slate-100 animate-fade-in-down rounded-b-2xl">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          {offre.description && (
            <div>
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Description du poste</h4>
              <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-line bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                {offre.description}
              </p>
            </div>
          )}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-4">À propos de l'entreprise</p>
            ...
            <button
              onClick={() => navigate(`/recruteur/${recruteur._id || 'unknown'}`)}
              className="w-full text-xs text-blue-600 font-semibold py-2 border border-blue-100 rounded-lg hover:bg-blue-50 transition-colors flex items-center justify-center gap-1 mb-6"
            >
              Voir le profil <ExternalLink className="w-3 h-3" />
            </button>

            <hr className="mb-6 border-slate-50" />
            ...
            {alreadyApplied ? (
              ...
              >
                {applying
                  ...
                  : <><FileText className="w-4 h-4" /> Postuler maintenant</>}
              </button>
            )}
          </div>
        </div>
...
export default function CandidateDashboard() {
  ...
  const [selectedOffre, setSelectedOffre] = useState(null); // Used for accordion toggle
  ...
  // ── Direct apply (non-AI, POST /api/candidature) ──────────────────────────
                ...
                <div key={offre._id} className={`bg-white rounded-2xl border transition-all ${isSelected ? 'border-blue-300 shadow-md ring-2 ring-blue-50' : 'border-slate-200 shadow-sm hover:border-blue-200'}`}>
                  ...
                  {isSelected && (
                    <OffreDetailPanel
                      offre={offre}
                      onApply={(id) => {
                        handleDirectApply(id);
                      }}
                      applying={applyingId === offre._id}
    ...
    if (activeTab === 'matches') return (
      <div className="space-y-6">
        <div className="flex justify-between items-end">
          ...
          <button onClick={loadMatch} disabled={matchLoading}
        ...
        {matchLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center space-y-3">
            ...
            <button onClick={loadMatch} className="block mx-auto text-blue-600 text-sm font-medium underline">
              Réessayer
            </button>
          </div>
        ) : matchData ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <button
                  onClick={() => navigate(`/recruteur/${matchData.offre.idRecruteur?._id || 'unknown'}`)}
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {matchData.offre.typeContrat && (
                <span className={`text-xs font-semibold px-3 py-1.5 rounded-xl border ${CONTRACT_COLOR[matchData.offre.typeContrat] || 'bg-slate-50 text-slate-600 border-slate-100'}`}>
                  {matchData.offre.typeContrat}
                </span>
              )}
              {matchData.offre.localisation && (
                <span className="flex items-center gap-1.5 text-xs text-slate-600 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                  <MapPin className="w-3 h-3" /> {matchData.offre.localisation}
                </span>
              )}
              {matchData.offre.salaireMin && matchData.offre.salaireMax && (
                <span className="flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100">
                  <Euro className="w-3 h-3" />
                  {matchData.offre.salaireMin.toLocaleString()} – {matchData.offre.salaireMax.toLocaleString()}
                </span>
              )}
            </div>
            ...
            <button
              onClick={() => handleMatchApply(matchData.offre._id)}
            ...
            >
              {matchApplying
                ...
                : <><Sparkles className="w-4 h-4" /> Postuler avec ce match IA</>}
            </button>
          </div>
        ) : (
    ...
    if (activeTab === 'applications') return (
      <div className="space-y-5">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">Suivez l'état de toutes vos candidatures.</p>
        </div>
                ...
                <div key={c._id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                  ...
                  {c.entretien?.dateEntretien && (
                    <div className="mt-3 flex items-center gap-2 text-sm text-blue-600 bg-blue-50 rounded-xl px-4 py-2.5 ml-14">
                      ...
                      Entretien le{' '}
                      {new Date(c.entretien.dateEntretien).toLocaleDateString('fr-FR', {
